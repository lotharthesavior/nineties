...
> SQLite-based EventStore implementation for nineties-core
...
`nineties-es-sqlite` provides a SQLite-based implementation of the `EventStore` trait from `nineties-core`. It offers a lightweight, embedded event store suitable for:
...
`SqliteEventStore` implements all methods from `nineties_core::EventStore`.
...
**Alternative**: Use `nineties-es-postgres` for distributed deployments.
...
- [Nineties Core README](../nineties-core/README.md)
...
Part of the [Nineties Framework](https://github.com/lotharthesavior/nineties).
